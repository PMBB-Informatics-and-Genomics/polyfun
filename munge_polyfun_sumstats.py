...
def rename_df_columns(df_sumstats):
    ...
    beta_col = find_df_column(df_sumstats, ['BETA', 'EFF', 'EFFECT', 'EFFECT_SIZE', 'OR', 'eaOR'], allow_missing=True)
    se_col = find_df_column(df_sumstats, ['SE'], allow_missing=True)
    pvalue_col = find_df_column(df_sumstats, ['p-value', 'P_BOLT_LMM', 'P', 'PVALUE', 'P-VALUE', 'P_value', 'PVAL'], allow_missing=True)
    z_col = find_df_column(df_sumstats, ['Z', 'ZSCORE', 'Z_SCORE'], allow_missing=True)    
    n_col = find_df_column(df_sumstats, ['N', 'OBS_CT', 'sample_size', 'NMISS', 'n_samples'], allow_missing=True)    
    ...
    try:
        allele1_col = find_df_column(df_sumstats, ['ALLELE1', 'A1', 'a1', 'a_1', 'ALT', 'Allele2', 'effect_allele', 'reference_allele', 'EA'])
        allele0_col = find_df_column(df_sumstats, ['ALLELE0', 'A0', 'a0', 'a_0', 'REF', 'Allele1', 'other_allele', 'other_allele', 'NEA'])
    except ValueError:
        allele1_col = find_df_column(df_sumstats, ['ALLELE1', 'A1', 'a1', 'a_1', 'ALT', 'effect_allele', 'alternate_allele'])
        allele0_col = find_df_column(df_sumstats, ['ALLELE2', 'A2', 'a2', 'a_2', 'REF', 'reference_allele'])
...
def sanity_checks(df_sumstats):
    ...
    #check for duplicates — drop instead of raise (GWAMA meta files can have duplicate positions)
